...
| URL                                                          | 说明                     | 是否允许通信 |
| ------------------------------------------------------------ | ------------------------ | ------------ |
| [www.domain.com/](http://www.domain.com/)a.js [www.domain.com/](http://www.domain.com/)b.js [www.domain.com/](http://www.domain.com/)lab/c.js | 同一域名，不同文件或路径 | 允许         |
| [www.domain.com](http://www.domain.com/):8000/a.js [www.domain.com/b.js](http://www.domain.com/b.js) | 同一域名，不同端口       | 不允许       |
...
  **jsonp**的原理就是利用``标签没有跨域限制，通过``标签src属性，发送带有callback参数的GET请求，服务端将接口返回数据拼凑到callback函数中，返回给浏览器，浏览器解析执行，从而前端拿到callback函数返回的数据。
1）原生JS实现：
